...
for code in rk.ind_code_list:
    if code not in ['C21', 'C24', 'C19', 'C20', 'C40', 'C41']:
        ...
        df_second = cluster_list[1].copy()
        for i in df_second["Stkcd"]:
            capital_code_list.append(i)  # 制作一个头部企业代码列表，包含聚类前两类的企业

        capital_df = ind.df[ind.df["Stkcd"].isin(capital_code_list)].copy()
        capital_df = capital_df[["year", "证券简称", "行业名称", "行业代码",
                                 "年个股总市值（千元及剔除B股）", "固定资产净额", "资产总计",
                                 "归属于母公司所有者权益合计", "所有者权益合计", "营业总收入",
                                 "营业收入", "利润总额"]]
        capital_df["year"] = capital_df["year"].astype("int")
        common_df = ind.df[~ind.df["Stkcd"].isin(capital_code_list)].copy()
        common_df["year"] = common_df["year"].astype("int")
        df_sum = pd.DataFrame()
        for year in rk.sample_period:
            df_year_capital = capital_df[capital_df["year"] == year].copy()
        ...
        panel_sub = pd.merge(common_df, df_sum, how="left", on="year")
        print(panel_sub)
